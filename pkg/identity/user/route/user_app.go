...
func InitUserApp(e *echo.Echo, cs *model.AppService, d *model.CommonDAO) {
	...
	r.POST("/sign-up", h.CreateUser, validation.CreateUserCommandValidation)
	r.POST("/log-in", h.LoginUser, validation.CreateLoginUserCommandValidation)
	r.GET("/me", h.GetDetailUser, middleware.CheckUser(d))
	r.PUT("/me/update", h.UpdateUser, middleware.CheckUser(d), validation.UpdateUserCommandValidation)
