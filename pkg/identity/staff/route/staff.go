...
func InitStaffAdmin(e *echo.Echo, cs *model.AdminService, d *model.CommonDAO) {
	...
	// only root
	r := e.Group("/staff")
	r.POST("", h.Create, middleware.CheckPermissionRoot(d), validation.CreateStaffCommandValidation)
	...
	r.PUT("/:staffID", h.UpdateRole, middleware.CheckPermissionRoot(d), h.StaffGetByID, validation.UpdateStaffRoleCommandValidation)
	r.DELETE("/:staffID", h.DeleteStaff, middleware.CheckPermissionRoot(d), h.StaffGetByID)
