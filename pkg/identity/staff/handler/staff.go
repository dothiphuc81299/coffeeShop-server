...
func (h *StaffAdminHandler) Create(c echo.Context) error {
	cc := util.EchoGetCustomCtx(c)
	var (
		body = c.Get("body").(model.CreateStaffCommand)
	)
...
func (h *StaffAdminHandler) UpdateRole(c echo.Context) error {
	cc := util.EchoGetCustomCtx(c)
	var (
		body  = c.Get("body").(model.UpdateStaffRoleCommand)
...
func (h *StaffAdminHandler) LoginStaff(c echo.Context) error {
	cc := util.EchoGetCustomCtx(c)
	var (
		body = c.Get("body").(model.LoginStaffCommand)
	)

	data, err := h.StaffService.LoginStaff(cc.GetRequestCtx(), body)
	if err != nil {
		return cc.Response400(nil, err.Error())
	}
	return cc.Response200(echo.Map{
